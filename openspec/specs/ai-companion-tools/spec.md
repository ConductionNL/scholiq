...
No new OpenRegister schemas — this capability reads existing `Course` and `Lesson` objects via `ObjectService`. The provider is a thin read-only adapter; persistence and lifecycle stay with `course-management`.

## Out of Scope

- Tools that touch learner data (Enrolment, Attestation, Credential, per-learner records) — deferred to a follow-up that wires proper per-student authorisation
- Dutch localisation of tool descriptions
- Tightening `requireCourseReadAccess()` to a Scholiq-specific group check
- A dynamic per-caller catalogue (the catalogue is hard-coded by design at this revision)
