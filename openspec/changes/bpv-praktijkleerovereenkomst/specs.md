...
GIVEN een leerbedrijf met status "erkend" maar erfde_kwalificaties_jsonb = [21010, 21020] (contains 21000 NOT)
...
THEN SHALL het systeem de ondertekening blokken en tonen: "Wachten op handtekening ouder/voogd. Mail is verstuurd naar [parent-email]."
...
THEN SHALL het systeem:
  1. Validate aantal_uren gegen BpvTraject.aantal_bpv_uren_vereist cumulatief (warning if overschrijd)
...
THEN SHALL het systeem:
  ...
  2. Prakijkopleider klikt knop → zet ondertekend_door_praktijkopleider_op, ondertekend_door_praktijkopleider_naam, hash
...
THEN SHALL het systeem:
  ...
  2. Toon verplicht veld: "Motivering (min 200 teken)" met vooraf-ingevulde redenen (ziekte > 4w, conflict, bedrijfssluitin, gedragsovertreding, etc)
...
### 1. Traject creation + SBB-check + POK-generation

```
BPV-coördinator → scholiq-bpv (create BpvTraject)
  ├→ SbbRecognitionGuard.check(leerbedrijf_id, student.crebo)
  │   └→ SbbRecognitionService.poll(SBB-adapter)
  │       └→ confirm erkend voor deze kwalificatie? YES / NO
  ├→ [if erkend] → create Praktijkleerovereenkomst status:concept
  └→ BpvTraject.status = "pok-in-ondertekening"

BPV-coördinator → "Generate POK"
  ├→ SbbRecognitionService.fetchTemplate(model-versie, leerwerktype)
  ├→ Populate with student, leerbedrijf, opleiding, leerdoelen
  └→ Generate sign-links → send mails (school, student, leerbedrijf, parent)

Student / Leerbedrijf / Parent → clicks sign-link → digitaal ondertekenen
  └→ Praktijkleerovereenkomst.ondertekening_*_datum filled
      ├→ PokSignatureGuard.check(isFullySigned)
      └→ [if fully] → BpvTraject.status transition "pok-in-ondertekening" → "actief"
```

### 2. Weekly hours registration + signature

```
Student (pipelinq) → register 8h Monday 2026-06-01
  └→ BpvUrenRegistratie.create(traject_id, datum, aantal_uren, activiteit, werkprocessen)
      ├→ [if student < 18] MinorArbourGuard.check() → arbo_check_result
      └→ status = "concept"

Student (pipelinq) → repeat Tue–Fri
  └→ 5 registraties week 23 in status "concept"

Friday 17:00 → pipelinq notifies prakijkopleider
  └→ Praktijkopleider → 1-knop "Ik onderteken week 23"
      └→ ondertekend_door_praktijkopleider_op = now
          ├→ All 5 registraties status → "goedgekeurd"
          └→ Student notified: "Week 23 ondertekend."
```

### 3. SBB recognition loss escalation

```
Daily: SbbRecognitionService.poll() → check Leerbedrijf.sbb_erkenning_status in SBB-register
  ├→ Leerbedrijf X status changed "erkend" → "ingetrokken"
  └→ Create SbbAuditEvent; query BpvTraject where leerbedrijf_id = X AND status = "actief"
      └→ found: [traject1, traject2, traject3]
          └→ Alert BPV-coördinator: "Leerbedrijf X erkenning ingetrokken. 3 actieve trajecten affected."
              ├→ Option A: Continue (audit-trail; risky)
              ├→ Option B: Transfer to different leerbedrijf (new POK)
