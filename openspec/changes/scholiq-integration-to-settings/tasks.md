...
- [x] Confirm no overlap with OpenRegister abstractions: this is a scholiq-local
...
- [x] Confirm `@conduction/nextcloud-vue` already provides the Settings/gear grouping
...
- [x] Confirm scholiq has NO `src/menu-layout.json` and NO `src/manifest.d/`
...
- [x] Verify Phase 0 against the manifest: `DataExchange` is integration plumbing
...
- [x] In `src/manifest.json`, change the `DataExchange` menu entry: add
...
- [x] Do NOT alter any `pages[]` entry for `/data-exchange/...`
...
- [x] In `src/manifest.json`, keep `XapiStatementsMenu` as `section: "settings"` and
...
- [x] Leave the `XapiStatements` / `XapiStatementDetail` pages (`readOnly`) untouched.

## Phase 3: Verify routability and gating

- [x] Assert every relocated entry's pages still resolve as deep links:
...
- [x] Assert `Compliance` (and all other learner/instructor/officer entries) remain in
      the primary navigation and that `Compliance` keeps its
      `compliance-officer`/`hr` `visibleIf` gate.
- [x] Assert no `pages[]`, schema, register fragment, route, or controller changed
      (git diff scoped to the `menu[]` array of `src/manifest.json`).

## Phase 4: Validate

- [x] `cd scholiq && openspec validate scholiq-integration-to-settings --strict` → exit 0.
