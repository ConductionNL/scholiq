...
- [ ] 1.1 Add `AccessibilityStatement` to `lib/Settings/scholiq_register.json`: `channelTitle`, `status`
...
- [ ] 1.2 Add `reviewOverdue` to `AccessibilityStatement.x-openregister-calculations`: pure JSON-logic
...
- [ ] 1.3 Add `AccessibilityStatement.x-openregister-notifications.reviewOverdue`: `calculatedChange` trigger
...
- [ ] 1.4 Add `AccessibilityStatement.x-openregister-authorization`: `create`/`update` restricted to
...
- [ ] 1.5 Add `AccessibilityLimitation`: `accessibilityStatementId` ($ref `AccessibilityStatement`),
...
- [ ] 1.6 Add `AccessibilityFeedback`: `reporterUserId`, `affectedSurface`, `description`, `severity` (enum,
...
- [ ] 2.1 Implement `OCA\Scholiq\Lifecycle\AccessibilityStatementPublishGuard`: refuse `draft → published`
...
- [ ] 2.2 Extend the same guard to refuse `draft → published` when `status: fully-compliant` and any
  ...
  - **acceptance_criteria**: `AccessibilityStatementPublishGuardTest::testOpenLimitationBlocksFullyCompliantStatus`
    passes
- [ ] 2.3 Wire `AccessibilityStatement.x-openregister-lifecycle.transitions.publish.requires` to the new
  guard class in `lib/Settings/scholiq_register.json`.
  - **spec_ref**: `specs/accessibility-conformance/spec.md#requirement-a-statement-must-not-publish-without-evaluation-evidence`

## 3. Frontend — manifest pages

- [ ] 3.1 Add `ScholiqAccessibilityStatement.vue` (`src/views/`): renders the published statement in the
...
- [ ] 3.2 Add generic declarative `src/manifest.json` index/detail pages for `AccessibilityLimitation`
  ...
  - **spec_ref**: `specs/accessibility-conformance/spec.md#requirement-known-limitations-must-be-evidence-backed-and-linked-from-the-published-statement`
- [ ] 3.3 Add generic declarative `src/manifest.json` index/detail pages for `AccessibilityFeedback` (route
  ...
  - **spec_ref**: `specs/accessibility-conformance/spec.md#requirement-any-authenticated-user-must-be-able-to-report-an-accessibility-barrier`
- [ ] 3.4 Add an `Accessibility` navigation entry (mirrors the `Compliance` nav entry,
...
- [ ] 4.1 Add `@axe-core/playwright` as a `devDependency` in `package.json`.
  - **acceptance_criteria**: `npm ls @axe-core/playwright` resolves
- [ ] 4.2 Add `tests/e2e/accessibility-axe-scan.spec.ts`: iterate a representative sample of `type: "index"`
  ...
  - **acceptance_criteria**: `npx playwright test accessibility-axe-scan.spec.ts` runs and reports
    violations per page
- [ ] 4.3 Add `tests/e2e/spec-coverage/accessibility-conformance.spec.ts` covering: the statement page
...
- [ ] 4.4 PHPUnit: `AccessibilityStatementPublishGuardTest` covering
...
- [ ] 5.1 Add a short `docs/` page (or extend the existing Compliance user-guide page, per
