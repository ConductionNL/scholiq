...
- [ ] Add `Course` schema per design §1.1 — including `x-openregister-lifecycle` (`draft → published → archived` with `CoursePublishGuard` precondition on publish), `x-openregister-calculations` (`lessonCount`, `isPublished`), `x-openregister-aggregations` (`enrolledLearners`, `completedLearners`). Validate via OR's schema-validation endpoint. References: `decidesk/lib/Settings/decidesk_register.json` ActionItem schema for the `aggregations` + `calculations` shapes we use. `lifecycle` shape follows [openregister#1470](https://github.com/ConductionNL/openregister/issues/1470) — no in-fleet working example yet.
