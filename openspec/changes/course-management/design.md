...
> **Declarative-vs-imperative decision (per [hydra ADR-031 §"How to apply this rule"](../../../../hydra/openspec/architecture/adr-031-schema-declarative-business-logic.md))** — every Course / Lesson state transition + count + status calculation + relation fits the `x-openregister-*` extensions. They land as JSON patches on `lib/Settings/scholiq_register.json`, not as PHP service classes. **In-fleet references**: `decidesk/lib/Settings/decidesk_register.json` demonstrates the `aggregations` + `calculations` + `relations` + `seeds` shapes we adopt. `lifecycle` is not yet demonstrated anywhere in the fleet — it follows the contract being added to OR via [openregister#1470](https://github.com/ConductionNL/openregister/issues/1470). The two services this change ships — `Cmi5ImporterService` and `ScormToXapiTranslator` — are ADR-031 exceptions (NLP / external-system contract).
>
