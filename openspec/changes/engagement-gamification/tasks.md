...
- [x] 1.1 Add `PointRule` to `lib/Settings/scholiq_register.json`: `name`, `kind` (`enrolment-completed` |
...
- [x] 1.2 Add `PointAward` (`appendOnly: true`): `learnerId`, `pointRuleId` ($ref `PointRule`), `points`
...
- [x] 1.3 Add `EngagementLevel`: `name`, `order` (integer), `minPoints` (number), `icon` (nullable string),
  ...
  - **acceptance_criteria**: rows are independently creatable/editable/orderable via the standard OR object
    UI.
- [x] 1.4 Add `LearnerEngagement` with `x-openregister: {active: true, hardDelete: false, searchable: true,
...
- [x] 1.5 Add `Leaderboard`: `name`, `cohortId` (nullable $ref `Cohort`), `courseId` (nullable $ref
...
- [x] 2.1 Add `OCA\Scholiq\Engagement\PointEngagementEvaluator`: `evaluate(string $learnerId, string
...
- [x] 2.2 Add `OCA\Scholiq\Listener\PointAwardTriggerHandler` (`IEventListener<ObjectTransitionedEvent>`):
...
- [x] 2.3 Add `OCA\Scholiq\Listener\LearnerEngagementRollupHandler` (`IEventListener<ObjectCreatedEvent>` on
...
- [x] 2.4 Add `OCA\Scholiq\Controller\LeaderboardController::getRankings(string $cohortId)`:
...
- [x] 2.5 Register the route in `appinfo/routes.php` for `LeaderboardController::getRankings`.
  - **spec_ref**: `specs/engagement/spec.md#requirement-a-ranked-leaderboard-is-opt-in-per-cohortcourse-default-off-and-respects-a-per-learner-opt-out`
...
- [x] 3.1 Add `src/manifest.json` index/detail pages for `PointRule`, `EngagementLevel`, `Leaderboard`
...
- [x] 3.2 Add a points/level KPI widget to the student dashboard (`ScholiqDashboards role="student"`),
...
- [x] 3.3 Add `src/views/LeaderboardView.vue` — the one custom view: renders `LeaderboardController`'s
...
- [x] 3.4 i18n: add all new `en`/`nl` translation keys (English keys, per project convention) for the new
...
- [x] 4.1 PHPUnit `PointEngagementEvaluatorTest`: totals sum, level resolution at exact thresholds, streak
  continuation across same-day/yesterday, streak reset across a gap.
  - **spec_ref**: `specs/engagement/spec.md#requirement-learner-totals-level-and-streak-are-computed-by-a-php-evaluator-not-a-sum-aggregation`
- [x] 4.2 PHPUnit `PointAwardTriggerHandlerTest`: each of the three transitions awards correctly; late
  submission awards nothing; republish-after-revise does not duplicate; no `peer-review` kind exists.
  - **spec_ref**: `specs/engagement/spec.md#requirement-points-are-awarded-only-for-real-already-firing-events`,
    `specs/engagement/spec.md#requirement-pointaward-creation-is-idempotent-and-immutable`
- [x] 4.3 PHPUnit `LearnerEngagementRollupHandlerTest`: streak-milestone bonus fires exactly once per
  crossing; recursion guard verified (bonus award does not re-trigger another bonus check).
  - **spec_ref**: `specs/engagement/spec.md#requirement-learner-totals-level-and-streak-are-computed-by-a-php-evaluator-not-a-sum-aggregation`
- [x] 4.4 PHPUnit `LeaderboardControllerTest`: no-active-`Leaderboard` refused; non-member refused; opted-out
...
- [ ] 4.5 Vitest for `LeaderboardView.vue` (renders seeded rankings; renders empty/no-active-leaderboard
  ...
  - **spec_ref**: `specs/engagement/spec.md#requirement-frontend-surfaces-a-private-pointslevel-widget-and-one-opt-in-leaderboard-view`
  - **status (resume verification, 2026-07-15)**: NOT DONE — left unchecked deliberately. This repo has zero
    Vitest/Jest infrastructure anywhere (`grep -r vitest\|jest package.json` → no matching scripts; no
    `vitest.config.*`; no `*.spec.js`/`*.test.js` file exists outside `node_modules`). Frontend testing in
    this app is Playwright-e2e-only (`tests/e2e/**`). The precedent this task cites, `BsaRiskDashboard.vue`
    (the prior sole custom-view exception), also carries no Vitest spec — confirming this is a project-wide
    gap, not something specific to this change. Adding a Vitest harness from scratch is out of scope for this
    resume (would require new devDependencies, config, and CI wiring untouched by every other custom view in
    the app) — flagging as a task-authoring mismatch with the app's actual test architecture rather than
    inventing infra to force the checkbox.
- [x] 4.6 Playwright `tests/e2e/spec-coverage/engagement.spec.ts` covering the two scenarios in the frontend
...
- [ ] 5.1 `openspec validate engagement-gamification --strict` clean; `composer check:strict` (PHPCS, PHPMD,
  ...
  - **status (resume verification, 2026-07-15)**: left unchecked only because of the 4.5 Vitest gap above
    (no Vitest harness exists in this repo). Everything else in this line item is verified green: `openspec
    validate engagement-gamification --strict` → "Change 'engagement-gamification' is valid"; `phpcs`/`phpmd`/
    `phpstan`/`psalm` scoped to the four new `lib/` files → 0 findings (repo-wide runs show pre-existing
    failures on unrelated files only, none touching this change); hydra gates 1-31, 47, 48, 50-52 PASS,
    including the two explicitly required by the resume brief (28 schema-property-titles, 30
    effective-manifest-crossref) plus 51/52; gate-46 spec-anchor-existence has 0 findings against this
    change's files after fixing two pre-existing wrong-anchor bugs in `LeaderboardController.php`/
    `LeaderboardControllerTest.php`/`LeaderboardView.vue` (was using the wrong slug form for a heading
    containing a `/`); gate-32/46/49's remaining repo-wide failures are 100% pre-existing and unrelated (see
    PR/handoff notes); PHPUnit `phpunit-unit.xml` → 548/548 green (519 baseline + 29 new engagement tests, 0
    regressions); `node tests/validate-register.js`/`validate-manifest.js`/`validate-json-strict.js` → all
    PASS; `npm run build` → exit 0 (only pre-existing bundle-size warnings, no compile errors).
