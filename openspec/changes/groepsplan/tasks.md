...
- [x] 1.1 Add `GroupPlan` to `lib/Settings/scholiq_register.json`: `cohortId` ($ref `Cohort`, required),
...
- [x] 1.2 Add `GroupPlanSubgroup`: `groupPlanId` ($ref `GroupPlan`, required), `name` (required),
...
- [x] 1.3 Add `GroupPlanEvaluation`: `groupPlanId` ($ref `GroupPlan`, required), `evaluatedAt`,
...
- [x] 1.4 Add nullable `originGroupPlanSubgroupId` ($ref `GroupPlanSubgroup`) to the existing
...
- [x] 2.1 Add `src/manifest.json` index+detail pages for `GroupPlan`, `GroupPlanSubgroup`,
...
- [x] 2.2 Add a nav entry ("Group plans") in `src/manifest.json`'s nav section near the existing
...
- [x] 3.1 Add `src/views/GroupPlanSubgroupLearnerContext.vue`: given the current `GroupPlanSubgroup`'s
...
- [x] 4.1 Add `tests/e2e/spec-coverage/groepsplan.spec.ts` (Playwright): a coordinator opens a seeded
...
- [x] 4.2 Add Dutch and English translations for all new i18n keys (notification subjects, view strings,
  manifest page titles).
  - **spec_ref**: all `learning-plan` groepsplan requirements
  - **acceptance_criteria**:
    - No hardcoded strings; `nl`/`en` both populated
- [x] 4.3 Add `x-openregister-seed` fixtures for `GroupPlan`/`GroupPlanSubgroup`/`GroupPlanEvaluation`
...
- [x] 5.1 `openspec validate groepsplan --strict` clean.
- [x] 5.2 `composer check:strict` on any touched PHP (none anticipated — this change is declarative schema
