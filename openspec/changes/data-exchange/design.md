...
### 1.7 Declarative-vs-imperative decision (ADR-031)

All lifecycle state machines, notifications, calculations, and relations are declared in `lib/Settings/scholiq_register.json` via `x-openregister-*` extensions:

- `x-openregister-lifecycle` on `DataExchangeJob` — all state transitions including the OSO `pending-parent-review` path. Guards reference PHP classes by name; OR resolves them.
- `x-openregister-notifications` on `DataExchangeJob` — `jobFinished` alert to `requestedBy` on terminal state.
- `x-openregister-calculations` on `DataExchangeJob` — `durationSeconds`, `successRate` derived fields.
- `x-openregister-relations` on `DataExchangeJob` — typed links to `DataMappingProfile` and `AttendanceFlag`.
- `x-openregister-lifecycle` on `DataMappingProfile` — draft → active → archived.

**ADR-031 exceptions (PHP required):**
1. `DataExchangeRunHandler` — external-system bridge: must call OpenConnector REST API. OR's schema engine cannot reach outside systems. Exception documented per ADR-031 §Exceptions (1).
2. `DataExchangeRunGuard` + `OsoDossierReviewGuard` — PHP lifecycle guards called *by* the lifecycle engine (not replacing it). Correct pattern per ADR-031 §"PHP guards remain a legitimate seam".
3. `AttendanceFlagCreationHandler` + `AttendanceFlagReportGuard` — updating existing PHP guards to wire in the new `DataExchangeJob` creation and succeeded-check logic.

### 1.8 Reuse Analysis (ADR-001 / ADR-022)

| Need | Reused abstraction | Notes |
|---|---|---|
| Object persistence | `ObjectService::saveObject()` / `findAll()` | No custom Entity/Mapper |
| Lifecycle transitions | `x-openregister-lifecycle` | State machine declared in register |
| Audit trail | OR audit trail via `x-openregister-lifecycle` | ADR-008 — no parallel substrate |
| Notifications | `x-openregister-notifications` | No custom NotificationService |
| Relations | `x-openregister-relations` | No custom link tables |
| Calculated fields | `x-openregister-calculations` | No custom service for durationSeconds / successRate |
| File attachments (artefactRef) | OR file attachment abstraction | ADR-022 |
| Frontend CRUD | `CnIndexPage` + `CnDetailPage` for DataMappingProfile + DataExchangeJob | No custom controllers |
...
### 3.1 DataMappingProfile seed objects (Dutch values)

```json
[
  {
    "name": "BRON/ROD Leerling Export v2",
    "target": "bron-rod",
    "direction": "export",
    "sourceSchema": "learner-profile",
    "targetSchema": "DUO:LeerlingV2",
    "fieldMappings": [
      { "scholiqField": "eckId", "targetField": "pseudoId", "transform": "bsn-to-pseudonym" },
      { "scholiqField": "dateOfBirth", "targetField": "geboortedatum", "transform": "date-iso8601" },
      { "scholiqField": "cohortId", "targetField": "brinNummer", "transform": "cohort-to-brin" },
      { "scholiqField": "givenName", "targetField": "voornaam", "transform": null },
      { "scholiqField": "familyName", "targetField": "achternaam", "transform": null }
    ],
    "validationProfile": "DUO:BRON-ROD-2025",
    "active": true,
    "lifecycle": "active"
  },
  {
    "name": "OSO Overstapdossier PO→VO",
    "target": "oso",
    "direction": "export",
    "sourceSchema": "learner-profile",
    "targetSchema": "OSO:DossierV3",
    "fieldMappings": [
      { "scholiqField": "eckId", "targetField": "leerlingNummer", "transform": "bsn-to-pseudonym" },
      { "scholiqField": "givenName", "targetField": "voornaam", "transform": null },
      { "scholiqField": "familyName", "targetField": "achternaam", "transform": null },
      { "scholiqField": "dateOfBirth", "targetField": "geboortedatum", "transform": "date-iso8601" }
    ],
    "validationProfile": "OSO:Edu-K-2024",
    "active": true,
    "lifecycle": "active"
  },
  {
    "name": "Leerplichtmelding Gemeente",
    "target": "leerplicht",
    "direction": "export",
    "sourceSchema": "attendance-record",
    "targetSchema": "Digikoppeling:LeerplichtmeldingV1",
    "fieldMappings": [
      { "scholiqField": "eckId", "targetField": "pseudoId", "transform": "bsn-to-pseudonym" },
      { "scholiqField": "absenceStart", "targetField": "begindatum", "transform": "date-iso8601" },
      { "scholiqField": "absenceEnd", "targetField": "einddatum", "transform": "date-iso8601" },
      { "scholiqField": "absenceReason", "targetField": "reden", "transform": null }
    ],
  ...
  {
    "name": "SURFconext Attribuutrelease HO",
    "target": "surfconext",
    "direction": "export",
    "sourceSchema": "learner-profile",
    "targetSchema": "SAML:SURFconextV3",
    "fieldMappings": [
      { "scholiqField": "eckId", "targetField": "urn:mace:dir:attribute-def:uid", "transform": null },
      { "scholiqField": "email", "targetField": "urn:mace:dir:attribute-def:mail", "transform": null },
      { "scholiqField": "institutionId", "targetField": "urn:mace:terena.org:attribute-def:schacHomeOrganization", "transform": null }
    ],
    "validationProfile": "SURFconext:AttributePolicy-2025",
    "active": true,
    "lifecycle": "draft"
  },
  {
    "name": "HR-systeem Verplichte Training Sync",
    "target": "hr",
    "direction": "sync",
    "sourceSchema": "enrolment",
    "targetSchema": "SCIM:UserGroupV2",
    "fieldMappings": [
      { "scholiqField": "learnerId", "targetField": "externalId", "transform": null },
      { "scholiqField": "courseId", "targetField": "groupId", "transform": null },
      { "scholiqField": "status", "targetField": "active", "transform": null }
    ],
    "validationProfile": null,
    "active": false,
    "lifecycle": "draft"
...
### 3.2 DataExchangeJob seed objects (Dutch values)
...
[
  {
    ...
    "scope": { "schema": "learner-profile", "filters": {}, "cohortId": "cohort-2025-2026", "period": "2025-2026" },
    "requestedBy": "admin@rijnlandlyceum.nl",
    "requestedAt": "2026-01-15T08:30:00Z",
    "startedAt": "2026-01-15T08:30:05Z",
    "finishedAt": "2026-01-15T08:31:42Z",
    "result": { "recordsProcessed": 487, "recordsAccepted": 485, "recordsRejected": 2, "validationReport": "2 records afgewezen: ontbrekend BRIN-nummer", "artefactRef": null },
    "connectorRunId": "bron-rod-run-20260115-083005",
    "errorMessage": null,
    "originFlagId": null,
    "lifecycle": "partial"
  },
  {
    ...
    "target": "oso",
    "mappingProfileId": null,
    "scope": { "schema": "learner-profile", "filters": { "learnerId": "leerling-jan-de-vries-2014" }, "cohortId": null, "period": null },
    "requestedBy": "mentor.vandenberg@basisschooldekiem.nl",
    ...
    "errorMessage": null,
    "originFlagId": null,
    "lifecycle": "pending-parent-review"
  },
  {
    "direction": "export",
    "target": "leerplicht",
    "mappingProfileId": null,
    "scope": { "schema": "attendance-record", "filters": { "learnerId": "leerling-sara-mahmoud-2013" }, "cohortId": null, "period": "2026-03" },
    "requestedBy": "verzuimcoordinator@vmboroosendaal.nl",
    "requestedAt": "2026-03-05T14:22:00Z",
    "startedAt": "2026-03-05T14:22:03Z",
    "finishedAt": "2026-03-05T14:22:09Z",
    "result": { "recordsProcessed": 1, "recordsAccepted": 1, "recordsRejected": 0, "validationReport": null, "artefactRef": "file://or-attachments/leerplichtmelding-sara-mahmoud-20260305.xml" },
    "connectorRunId": "leerplicht-run-20260305-142203",
    "errorMessage": null,
    "originFlagId": "flag-sara-mahmoud-drempel-2026-03",
    "lifecycle": "succeeded"
  },
  {
    "direction": "sync",
    "target": "hr",
    "mappingProfileId": null,
    "scope": { "schema": "enrolment", "filters": { "courseType": "mandatory-training" }, "cohortId": null, "period": null },
    "requestedBy": "l-en-d@conduction.nl",
    "requestedAt": "2026-05-19T23:00:00Z",
    "startedAt": "2026-05-19T23:00:05Z",
    "finishedAt": "2026-05-19T23:01:18Z",
    "result": { "recordsProcessed": 312, "recordsAccepted": 312, "recordsRejected": 0, "validationReport": null, "artefactRef": null },
    "connectorRunId": "hr-sync-run-20260519-230005",
    "errorMessage": null,
    "originFlagId": null,
    "lifecycle": "succeeded"
  },
  {
    ...
    "target": "oso",
    "mappingProfileId": null,
    "scope": { "schema": "learner-profile", "filters": { "learnerId": "leerling-fatima-el-amrani-2014" }, "cohortId": null, "period": null },
    "requestedBy": "decaan.smits@wolfertvanborssele.nl",
    "requestedAt": "2026-04-22T09:05:00Z",
    "startedAt": "2026-04-22T09:05:02Z",
    "finishedAt": "2026-04-22T09:05:07Z",
    "result": { "recordsProcessed": 1, "recordsAccepted": 1, "recordsRejected": 0, "validationReport": null, "artefactRef": "file://or-attachments/oso-import-fatima-el-amrani-20260422.xml" },
    "connectorRunId": "oso-import-run-20260422-090502",
    "errorMessage": null,
    "originFlagId": null,
    "lifecycle": "succeeded"
  }
...
## 4. PHP — ADR-031 legitimate exceptions

### 4.1 DataExchangeRunHandler (lib/Listener/DataExchangeRunHandler.php)
...
### 4.2 DataExchangeRunGuard (lib/Lifecycle/DataExchangeRunGuard.php)
...
### 4.3 OsoDossierReviewGuard (lib/Lifecycle/OsoDossierReviewGuard.php)
...
### 4.4 AttendanceFlagCreationHandler (updated)
...
### 4.5 AttendanceFlagReportGuard (updated)
