...
- [ ] 1.1 Add `Room` to `lib/Settings/scholiq_register.json`: `name`, `capacity` (integer, minimum 1),
...
- [ ] 1.2 Add to `Session`: `roomId` (nullable, `$ref Room`), `externalRef` (nullable string),
...
- [ ] 1.3 Add `substitute-teacher` self-loop transition to `Session.x-openregister-lifecycle.transitions`
...
- [ ] 1.4 Add `x-openregister-relations`: `Session.roomId` → `Room`; update the school-structure register
...
- [ ] 2.1 Add `TimetableConflict`: `kind` (`teacher-double-booking|room-double-booking|
...
- [ ] 2.2 Add `ExamAccommodation`: `learnerId`, `assessmentId` (nullable, `$ref Assessment`),
...
- [ ] 2.3 Add `DataMappingProfile` entries (existing schema, new rows) for `target: timetable-import`
...
- [ ] 3.1 `OCA\Scholiq\Lifecycle\SessionChangeGuard` — refuses `cancel`/`substitute-teacher` unless the
...
- [ ] 3.2 `OCA\Scholiq\Listener\SessionChangeNoticeHandler` — on `cancel`/`substitute-teacher`, resolves
...
- [ ] 3.3 `OCA\Scholiq\Timetabling\TimetableConflictDetector` + `OCA\Scholiq\Listener\
...
- [ ] 3.4 `OCA\Scholiq\Timetabling\TimetableImportHandler` — `DataExchangeJob` execution handler for
...
- [ ] 4.1 `src/manifest.json`: index/detail pages for `Room`, `TimetableConflict`, `ExamAccommodation`.
  - **spec_ref**: `specs/timetabling/spec.md#requirement-frontend-is-declarative-with-named-custom-views`
- [ ] 4.2 `src/dialogs/SubstitutionModal.vue` — mark a Session cancelled or assign a substitute teacher,
  requiring `changeReasonKind` (and `substituteTeacherId` for substitution) before submit.
  - **spec_ref**: `specs/timetabling/spec.md#requirement-frontend-is-declarative-with-named-custom-views`
- [ ] 4.3 `src/views/TimetableConflictQueue.vue` — coordinator review surface for `open`/`acknowledged`
...
- [ ] 4.4 Extend `src/views/MyTimetable.vue` and `src/api/timetable.js` to render `roomId`/Room detail,
...
- [ ] 5.1 Extend `lib/Controller/TimetableController.php::fetchSessions()` to project `roomId` (+ resolved
  ...
  - **acceptance_criteria**: existing PHPUnit tests for `mine()` still pass unmodified; new assertions cover
    the added fields
- [ ] 5.2 Add a `changes` computation to `TimetableController::mine()`: caller's cohorts' Sessions whose
...
- [ ] 5.3 Confirm no new write path was added to `TimetableController` (read-only invariant preserved).
  - **spec_ref**: `specs/personal-timetable/spec.md#requirement-the-timetable-is-a-read-surface-only-over-existing-objects`

## 6. Tests

- [ ] 6.1 PHPUnit: `SessionChangeGuardTest`, `SessionChangeNoticeHandlerTest`, `TimetableConflictDetectorTest`
  (all six conflict kinds + idempotent rescan), `TimetableImportHandlerTest` (create/update/no-duplicate/
  manual-Session-untouched).
- [ ] 6.2 PHPUnit: `TimetableControllerTest` additions for the new projected fields and the `changes` list.
- [ ] 6.3 Register-validation suite: confirm `Room`, `TimetableConflict`, `ExamAccommodation`, and the
...
- [ ] 7.2 File a follow-up against the `assessment` capability to wire `ExamAccommodation`'s effective time
  ...
  accommodations").
- [ ] 7.3 Update `docs/ARCHITECTURE.md` with `Room`, `TimetableConflict`, `ExamAccommodation`, and the
