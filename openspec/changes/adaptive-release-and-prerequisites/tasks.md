...
- [x] 1.1 Add `prerequisiteCourseIds` to `Course` in `lib/Settings/scholiq_register.json` (`:810+` region):
...
- [x] 1.2 Add `releaseConditions` and `availableAfterDays` to `Lesson` (`:1080+` region): `releaseConditions`
...
- [x] 1.3 Add `releaseConditions` and `availableAfterDays` to `Assessment` (`:4640+` region), identical shape
...
- [x] 2.1 Add `OCA\Scholiq\Listener\EnrolmentPrerequisiteListener` (SPDX docblock; `@spec` tag): subscribes
...
- [x] 2.2 Register `EnrolmentPrerequisiteListener` on `ObjectCreatingEvent` in `lib/AppInfo/Application.php`
...
- [x] 3.1 Add `OCA\Scholiq\Release\LessonReleaseEvaluator` (ADR-031 stateless-service exception; SPDX; `@spec`
...
- [x] 3.2 Add `OCA\Scholiq\Controller\LessonReleaseController` (mirrors
...
- [x] 3.3 Register `['name' => 'lessonRelease#status', 'url' => '/api/lessons/{lessonId}/release-status',
...
- [x] 4.1 `src/views/LessonPlayer.vue`: before rendering any `contentType` (text, video, scorm12/2004, cmi5,
...
- [x] 4.2 Verify `Course`'s manifest create/edit form (`src/manifest.json`) surfaces `prerequisiteCourseIds`
...
- [x] 4.3 `releaseConditions` (array of objects with a `kind` discriminator) on `Lesson`/`Assessment`: verify
...
- [x] 5.1 PHPUnit for `EnrolmentPrerequisiteListener`, `LessonReleaseEvaluator`, `LessonReleaseController`
...
- [x] 5.2 Add `tests/e2e/spec-coverage/adaptive-release.spec.ts` (Playwright) covering the `@e2e`-tagged
  ...
  - **acceptance_criteria**: test passes against a seeded dev instance; matches the `@e2e` references in the
    spec scenarios
- [x] 5.3 Add Dutch and English translations for all new i18n keys introduced by the `LessonPlayer.vue`
