# Parent Conferences — New Capability

**Spec refs**: new capability `parent-conferences`; consumes `school-structure` (`Cohort`,
`LearnerProfile`), `portal-identity`'s additive UUID-ref convention (`learnerRef`/`guardianRef`),
ADR-022 (apps consume OR abstractions), ADR-031 (declarative notifications + narrowly-scoped
lifecycle guard/handler exceptions).
