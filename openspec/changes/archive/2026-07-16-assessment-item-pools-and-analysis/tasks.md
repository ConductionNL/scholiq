...
- [x] 1.1 Add `itemSelectionMode` (`fixed` | `random-draw`, default `fixed`) and `itemPoolConfig` (nullable
...
- [x] 1.2 Add `shuffleItemOrder` (boolean, default `false`) and `shuffleAnswerOptions` (boolean, default
...
- [x] 1.3 Add `variantGroupId` (nullable, `format: uuid`) to `Item` (`4472-4613` region).
  - **spec_ref**: `specs/assessment/spec.md#requirement-assessment-supports-a-pooled-random-item-draw-as-an-alternative-to-a-fixed-item-list`
- [x] 1.4 Add `drawnItemRefs` (array of `{itemId: uuid, points: number, optionOrder: string[] | null}`,
...
- [x] 2.1 Add `ItemStatistics` object to `lib/Settings/scholiq_register.json`: `itemId` ($ref `Item`),
...
- [x] 2.2 Add `AssessmentReliability` object: `assessmentId` ($ref `Assessment`), `sampleSize` (integer),
...
- [x] 2.3 Add `ItemRevisionFlag` object (`appendOnly: true`): `itemId` ($ref `Item`), `itemStatisticsId`
  ...
  - **acceptance_criteria**: `appendOnly: true`; lifecycle transitions match; notification recipients
    present in `nl`/`en`
- [x] 2.4 Add a nullable `Assessment.itemAnalysisConfig` object (`minSampleSize` default `20`,
...
- [x] 3.1 `lib/Listener/AssessmentDrawResolver.php` — `ObjectCreatedEvent` listener filtered to
...
- [x] 3.2 Register `AssessmentDrawResolver` on `ObjectCreatedEvent` in `lib/AppInfo/Application.php`,
  alongside the existing `XapiCompletionHandler` registration, with the same explanatory comment style.
  - **spec_ref**: same as 3.1
- [x] 3.3 Extend `lib/Lifecycle/AssessmentPublishGuard.php`: when `itemSelectionMode: random-draw`, accept
...
- [x] 4.1 `lib/Service/ItemAnalysisService.php` — stateless computation: `computeItemStatistics(itemId,
  ...
  - **spec_ref**: `specs/assessment/spec.md#requirement-per-item-statistics-are-computed-from-graded-results-gated-by-a-minimum-sample-size`
- [x] 4.2 `lib/Listener/ItemAnalysisRecomputeHandler.php` — `ObjectTransitionedEvent` listener filtered to
  ...
  - **spec_ref**: `specs/assessment/spec.md#requirement-a-quality-threshold-breach-opens-an-itemrevisionflag-routed-to-the-exam-board`
- [x] 4.3 Register `ItemAnalysisRecomputeHandler` on `ObjectTransitionedEvent` in
  `lib/AppInfo/Application.php`, alongside `GradeRollupHandler`'s registration on the same event class.
  - **spec_ref**: same as 4.2

## 5. Frontend

- [x] 5.1 `src/views/TakeAssessmentView.vue`: `loadItems()` reads `AssessmentResult.drawnItemRefs` (once
  ...
  - **spec_ref**: `specs/assessment/spec.md#requirement-every-assessmentresult-persists-a-frozen-server-resolved-snapshot-of-what-was-presented`
- [x] 5.2 `src/views/TakeAssessmentView.vue`: `createResult()` re-fetches the created `AssessmentResult` by
  ...
  - **spec_ref**: `specs/assessment/spec.md#requirement-item-draw-and-shuffle-resolution-runs-server-side-and-never-trusts-a-client-supplied-value`
- [x] 5.3 New `src/views/ItemAnalysisView.vue`: renders `ItemStatistics` (p-value, item-total correlation,
...
- [x] 5.4 `src/manifest.json`: register `ItemAnalysisView` as a named custom view; add declarative
...
- [x] 6.1 `tests/Unit/Settings/AssessmentItemPoolsRegisterTest.php` — schema shape assertions for the
  `Assessment`/`Item`/`AssessmentResult` deltas and the three new objects (mirrors
  `SecureExamTestModeTest.php`'s style).
- [x] 6.2 `tests/Unit/Listener/AssessmentDrawResolverTest.php` — draw count, subjectTags/difficulty
...
- [x] 6.3 `tests/Integration/Lifecycle/AssessmentPublishGuardRandomDrawIntegrationTest.php` — publish
...
- [x] 6.4 `tests/Unit/Service/ItemAnalysisServiceTest.php` — p-value and corrected item-total correlation
...
- [x] 6.5 `tests/Unit/Listener/ItemAnalysisRecomputeHandlerTest.php` — recompute fires on `graded`
