...
- [x] 1.1 Add `FeeItem` to `lib/Settings/scholiq_register.json`: `name`, `description`, `kind`
...
- [x] 1.2 Add `Order` to `lib/Settings/scholiq_register.json`: `payerKind` (`guardian | learner |
...
- [x] 1.3 Add `OrderLine` to `lib/Settings/scholiq_register.json`: `orderId` (required $ref `Order`),
...
- [x] 2.1 Add `Entitlement` to `lib/Settings/scholiq_register.json`: `feeItemId` (required $ref
...
- [x] 2.2 Add `PaymentTransaction` to `lib/Settings/scholiq_register.json`: `orderId` (required $ref
...
- [x] 3.1 Add `OCA\Scholiq\Lifecycle\FeeItemVoluntaryEntitlementGuard` (SPDX; `@spec` tag): on
...
- [x] 3.2 Add `OCA\Scholiq\Lifecycle\EntitlementOrderPaidGuard` (SPDX): on `Entitlement`'s `pending →
...
- [x] 3.3 Add `OCA\Scholiq\Lifecycle\OrderTotalValidationGuard` (SPDX): on `Order`'s `draft → open`
...
- [x] 3.4 Add `OCA\Scholiq\Listener\PaymentTransactionStatusHandler` (SPDX; mirrors `GradeRollupHandler`'s
...
- [x] 3.5 Add `OCA\Scholiq\Controller\PaymentTransactionController`: `initiate(orderId)` — creates a
...
- [x] 4.1 Add `src/manifest.json` index/detail pages for `FeeItem`, `Order`, `OrderLine`,
...
- [x] 4.2 Add `src/views/OrderPaymentPanel.vue`: for an `open`/`partially-paid` `Order`, shows its
...
- [x] 5.1 PHPUnit for `FeeItemVoluntaryEntitlementGuard`, `EntitlementOrderPaidGuard`,
...
- [x] 5.2 Add `tests/e2e/spec-coverage/payments.spec.ts` (Playwright): a payer opens `OrderPaymentPanel`
...
- [x] 5.3 Add Dutch and English translations for all new i18n keys (ADR-005): `OrderPaymentPanel.vue`
...
- [x] 6.1 `openspec validate school-payments --strict` clean; PHPUnit green for all five new PHP classes;
