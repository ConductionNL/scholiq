...
- [x] 1.1 Add `DossierNote` to `lib/Settings/scholiq_register.json`: `learnerId`, `authorId`, `date`
...
- [x] 1.2 Add `x-openregister-authorization.create: ["admin", "mentor", "coordinator"]` and
...
- [x] 1.3 Add `x-openregister-processing` to `DossierNote`: `code: scholiq-pupil-dossier-notes`,
...
- [x] 1.4 Add `BehaviourIncident` to `lib/Settings/scholiq_register.json`: `learnerId`, `reportedBy`,
...
- [x] 1.5 Add `x-openregister-authorization.create: ["admin", "mentor", "coordinator"]` and
  `x-property-rbac.read` (`anyOf`: `role: admin`, `role: mentor`, `role: coordinator`, `match: field=
...
- [x] 1.6 Add `x-openregister-notifications.incidentRecorded` to `BehaviourIncident`: `trigger: {type:
...
- [x] 1.7 Add `x-openregister-processing` to `BehaviourIncident`: `code: scholiq-behaviour-incidents`,
...
- [x] 1.8 Add `WellbeingCheckIn` to `lib/Settings/scholiq_register.json`: `learnerId`, `submittedAt`
...
- [x] 1.9 Add `x-property-rbac.read` (`anyOf`: `role: admin`, `role: mentor`, `role: coordinator`, `match:
...
- [x] 1.10 Add `x-openregister-processing` to `WellbeingCheckIn`: `code: scholiq-wellbeing-checkins`,
...
- [x] 2.1 Add `src/manifest.json` index/detail pages for `DossierNote`, `BehaviourIncident`,
...
- [x] 2.2 Add three `object-list` widgets to `LearnerProfileDetail` (`lprof-dossier-notes`,
...
- [x] 2.3 Add `src/views/PupilDossierTimelineView.vue`: fetches and chronologically merges `DossierNote`,
...
- [x] 3.1 Add `tests/e2e/spec-coverage/pupil-dossier.spec.ts` (Playwright): staff creates a `DossierNote`
  ...
  - **acceptance_criteria**:
    ...
    - **Status note (resume verification, 2026-07-15)**: file written mirroring the established
      eportfolio.spec.ts/competency-framework.spec.ts lightweight route-resolution smoke pattern (no
      seeded-CRUD flow — matches every other spec-coverage file in this repo); `--list` confirms all 6
      tests parse; `gate-19` (e2e-coverage) passes. NOT executed against a live instance this session —
      the only reachable dev container's `scholiq` bind-mount is a different checkout
      (`nextcloud-docker-dev/workspace/server/apps-extra/scholiq`), not this working tree, and deploying
      this branch into that shared container was out of scope per the no-deploy-to-shared-dev-instance
      rule. Live execution remains outstanding.

- [x] 3.2 PHPUnit schema-validation coverage for the three new schemas' `x-property-rbac`/
...
- [x] 3.3 Add Dutch and English translations for all new i18n keys (notification subjects, manifest page
...
- [x] 3.4 File an OpenRegister platform-capability issue for row-conditional `x-property-rbac` (a `match`
  ...
  - **acceptance_criteria**:
    ...
    - Filed: [codeberg.org/Conduction/openregister#416](https://codeberg.org/Conduction/openregister/issues/416);
      linked from `design.md` Decision 1
...
- [ ] 4.1 `openspec validate pupil-dossier-notes --strict` clean; PHPUnit green; no dangling `$ref`s in the
  ...
  - **acceptance_criteria**:
    ...
    - **Status note (resume verification, 2026-07-15)**: `openspec validate pupil-dossier-notes --strict`
      → "Change 'pupil-dossier-notes' is valid". PHPUnit `phpunit-unit.xml` → 519/519 green (2559
      assertions; +8 tests / +251 assertions over baseline 511/2308), 1 pre-existing coverage-driver
      warning only. No dangling `$ref`s (verified programmatically across all 83 schemas). `hydra` gates
      28/30/51/52 all PASS. Remaining unchecked because NOT done this session: Playwright
      `pupil-dossier.spec.ts` was not executed live (see 3.1 status note — no matching dev container
      available), and the RBAC-floor denial was therefore only asserted in PHPUnit
      (`PupilDossierNotesRegisterTest`), not re-verified against a running instance. Leave this box
      unchecked until both are done against a live, correctly-mounted dev instance.
