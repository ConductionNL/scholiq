...
# AI Surface Information Architecture

**Status:** proposed
**Scope:** scholiq
**Tier:** leaf-app
**Depends on:** ADR-034 (AI chat companion), ADR-037 (modular config fragments / canonical REQ-ID), ADR-012 (deduplication); scholiq `AiFeature` schema; `src/manifest.json`; `src/views/ScholiqSettings.vue`

## Purpose

Collapse the two adjacent AI-labelled top-level nav entries ("AI features" governance register and "Assistant" chat companion) into a single coherent AI surface: one interactive "Assistant" nav entry, with the EU AI Act feature register re-homed under Settings while every page stays routable.
