...
- [x] 1.1 In `appinfo/info.xml`, change `<licence>agpl</licence>` to `<licence>EUPL-1.2</licence>` (Edit tool).
...
- [x] 1.2 Verify: `grep -r '<licence>agpl' appinfo/` returns nothing; `openspec validate fix-manifest-licence-eupl --strict` is clean.
  - **spec_ref**: `specs/app-metadata/spec.md#requirement-the-manifest-licence-matches-the-repositorys-actual-licence`
  - **acceptance_criteria**:
