...
**Scope**: scholiq
**OpenSpec changes**:
- `openspec/changes/competency-framework/`

## Purpose

Every incumbent this app competes with — Canvas Outcomes+Mastery, Moodle's
Competency Framework, Brightspace, Cornerstone's skills graph, SAP
SuccessFactors, 360Learning, eFront, Opigno, ILIAS, Chamilo, It's Learning,
Coursera, Credly, ExamSoft — ships a first-class competency/learning-outcome
taxonomy that content and assessments align to, and that rolls a learner's
evidence up into a mastery signal. Scholiq has none: `WerkprocesAssessment`
carries four flat kwalificatiedossier strings with no shared object behind
them, `Lesson.learningObjectives` and `Item.subjectTags` are untyped free
text, and nothing lets a `Course`, `Assignment`, `Assessment`, or `Credential`
declare what it teaches or assesses in a way another capability can query.
This spec adds that taxonomy — `CompetencyFramework` (the named scale:
SBB kwalificatiedossier, SLO kerndoelen/eindtermen, ESCO, or school-defined),
`Competency` (a recursive node in that taxonomy — kerntaak → werkproces,
leerlijn → kerndoel, or any other two-or-more-level hierarchy), and
`CompetencyAttainment` (the declared, event-driven per-learner roll-up) — and
generalizes `WerkprocesAssessment`'s existing kwalificatiedossier fields into
it rather than duplicating a second taxonomy.
...
- **Performance**: `CompetencyAttainment` recomputation is event-driven (one handler invocation per
  qualifying transition), never a scheduled batch scan — matches `GradeRollupHandler`'s cost profile.
- **Accessibility**: `SkillsGapDashboard.vue` follows the same NcSelect `inputLabel`/WCAG AA conventions as
  every other custom view in this app (ADR-004).
- **Internationalization**: all new i18n keys (`en`/`nl`) source in English per fleet policy; `nl`
  catalogue entries populated by the existing extraction pipeline (see `bsa-study-progress-guard`'s
  documented precedent for why `l10n/*.json` is not hand-edited).

## Acceptance Criteria

- Register-validation (`npm run check:register`) and manifest-validation (`npm run check:manifest`) pass
  with the three new schemas, the eight additive-property schemas, and the new manifest pages.
- PHPUnit coverage on `CompetencyAttainmentRollupHandler` proves: competency-aligned `GradeEntry` publish
  creates/updates the roll-up; competency-aligned `WerkprocesAssessment` confirm creates/updates the
  roll-up; unaligned evidence is a no-op; re-processing the same evidence id is idempotent (no duplicate
  entries in the evidence-id arrays).
- `openspec validate competency-framework --strict` passes.

## Notes

- `LearningOutcome` is deliberately not a schema — see design.md's "Rejected alternatives" for the
  two-schema design this change did not take and why.
- Depends on nothing new in this repo (`bpv`, `course-management`, `assignments`, `assessment`,
  `certification`, `school-structure` are all already-shipped capabilities this change adds to). An
  ESCO/SLO/SBB import connector populating these schemas from an external source is explicit
  cross-repo/`openconnector` follow-up, not built here.
