...
- [x] 1.1 Add `CompetencyFramework` schema to `lib/Settings/scholiq_register.json`: `name`,
      ...
      `Course`).
- [x] 1.2 Add `Competency` schema: `frameworkId` (`$ref: CompetencyFramework`), `parentId` (nullable,
...
- [x] 1.3 Add `Competency.childCount` (`x-openregister-aggregate-refs` count where `parentId ==
...
- [x] 2.1 Add `CompetencyAttainment` schema, `readOnly: true`, no lifecycle (mirrors `FinalGrade`):
...
- [x] 2.2 Add `x-property-rbac.read` on `CompetencyAttainment`: `anyOf` `admin`/`hr`/`manager` roles, or
      `learnerId == $userId` — mirrors `FinalGrade`'s shape, extended per `dashboard/spec.md:22`'s named
      audience.
- [x] 2.3 Add `competencyIds` (array uuid `$ref: Competency`, default `[]`) to `Course` and `Lesson`;
      bump both schemas' own `version`.
- [x] 2.4 Add `competencyIds` to `Assignment`; bump its `version`.
- [x] 2.5 Add `competencyIds` to `Assessment` and `Item`; bump both schemas' own `version`.
- [x] 2.6 Add `competencyIds` to `Credential`; bump its `version`.
- [x] 2.7 Add `requiredCompetencyIds` (array uuid `$ref: Competency`, default `[]`) to `Programme`; bump
      its `version`.
- [x] 2.8 Add `competencyId` (nullable, `$ref: Competency`) to `WerkprocesAssessment`; bump its `version`.
      ...
      Considerations).
- [x] 2.9 Bump `lib/Settings/scholiq_register.json`'s `info.version` `0.7.0 → 0.8.0`.
- [x] 2.10 Register-validation test: all new/touched schemas + the version bump validate against `npm run
       check:register`.

## 3. Backend — CompetencyAttainmentRollupHandler

- [x] 3.1 Create `lib/Listener/CompetencyAttainmentRollupHandler.php` (`IEventListener`, SPDX header,
      ...
      shape.
- [x] 3.2 Implement the `GradeEntry` `publish`/`republish` branch: filter to `sourceKind:
...
- [x] 3.3 Implement the `GradeEntry` `publish`/`republish` branch for `sourceKind: assessment-result`:
...
- [x] 3.4 Implement the `WerkprocesAssessment` `confirm` branch: read the assessment's own `competencyId`
...
- [x] 3.5 No-op paths: `GradeEntry` sourceKind ∉ {assignment-submission, assessment-result}; a source with
...
- [x] 3.6 Add the `competencyId` server-side resolution to `WerkprocesAssessment` creation: match
...
- [x] 3.7 Register `CompetencyAttainmentRollupHandler` in `lib/AppInfo/Application.php` alongside the
...
- [x] 4.1 Add `src/manifest.json` read-only index/detail pages for `CompetencyFramework`, `Competency`,
      `CompetencyAttainment` (no create/edit actions rendered for `CompetencyAttainment` — system-derived).
- [x] 4.2 Add `src/views/SkillsGapDashboard.vue`: computes the required set (`Programme.
...
- [x] 4.3 Manifest validation: `npm run check:manifest` passes.

## 5. Tests and docs

- [x] 5.1 PHPUnit `tests/Unit/Listener/CompetencyAttainmentRollupHandlerTest.php`: assignment-submission
...
- [x] 5.2 Add `tests/e2e/spec-coverage/competency-framework.spec.ts` (Playwright): a learner/manager opens
...
- [x] 5.3 Add Dutch and English source strings for `SkillsGapDashboard.vue` and any new notification
      subjects (none are declared by this change — `CompetencyAttainment` has no
      `x-openregister-notifications` block).
- [x] 5.4 Run `openspec validate competency-framework --strict` and resolve any reported issues before
      this change is considered ready to move to implementation.

## 6. Verify

- [x] 6.1 `openspec validate competency-framework --strict` clean; PHPUnit green for
