...
- [ ] 1.1 Add `SovereigntyPolicy` to `lib/Settings/scholiq_register.json`: `policy` (enum
...
- [ ] 1.2 Add `SovereigntyPolicy.x-openregister-authorization`: `create`/`update` restricted to
  `["admin", "compliance-officer"]` (mirrors `AccessibilityLimitation`).
  - **spec_ref**: `specs/ai-locality-guarantee/spec.md#requirement-the-system-shall-let-a-school-declare-an-ai-processing-locality-policy`

## 2. Backend — classifier + policy service

- [ ] 2.1 Implement `OCA\Scholiq\Service\AiLocalityClassifier::classify(string $chatProvider, ?string
  ...
  - **acceptance_criteria**:
    ...
    - No branch of this method ever returns `verified: true` for `locality: on-premises` or `locality:
      eu-hosted`
- [ ] 2.2 Implement `OCA\Scholiq\Service\SovereigntyPolicyService::currentPolicy(): string` (reads the
...
- [ ] 3.1 Modify `OCA\Scholiq\Lifecycle\AssessmentPublishGuard::check()`: inject `AiLocalityClassifier` and
  ...
  - **acceptance_criteria**:
    ...
    - Existing `AssessmentPublishGuardTest` DPO-enablement and manual-proctoring coverage remains green
      unmodified
- [ ] 3.2 Implement `OCA\Scholiq\Controller\AiProcessingDisclosureController::index()` composing Hermiq's
...
- [ ] 4.1 Add `ScholiqAiProcessingDisclosure.vue` (`src/views/`): renders the `SovereigntyPolicy` tier with an
...
- [ ] 4.2 Add a navigation entry gated to `admin`/`compliance-officer` (`visibleIf`, mirrors the `Compliance`
...
- [ ] 6.1 Amend `ADR-005-eu-ai-act-gating.md` with a short addendum (mirrors its existing 2026-07-06
